...
(function(){
  ...
  bloquearVotacaoBtn?.addEventListener('click', async () => {
    if(!confirm('Deseja bloquear a votação?\n\nOs convidados não poderão mais votar.')){
      return;
    }
  ...
  exportarCsvBtn?.addEventListener('click', async () => {
    ...
    // Gerar CSV
    let csv = 'Nome,CPF,Telefone,Email,Dependentes Adultos,Dependentes Crianças,Total Pessoas,Data Confirmação\n';
    ...
    result.rsvps.forEach(rsvp => {
      ...
      csv += `"${rsvp.nome}","${rsvp.cpf}","${rsvp.telefone || ''}","${rsvp.email || ''}",${adultos},${criancas},${total},"${new Date(rsvp.created_at).toLocaleString('pt-BR')}"\n`;
      
      // Adicionar linhas de dependentes
      dependentes.forEach(dep => {
        csv += `"  └ ${dep.nome} (${dep.tipo === 'adulto' ? 'Adulto' : 'Criança ' + dep.idade + ' anos'})","","","","","","",""\n`;
      });
    });
